...
lmProfile <- rfe(x=xx.train[, 1:12], y=xx.train$Class,
                 sizes = subsets,
                 rfeControl = ctrl)
...
boot.ctrl = trainControl ( method = "cv" , number = 10, sampling = "rose", summaryFunction = twoClassSummary, classProbs = TRUE )
set.seed(123)
rf.fit = train ( Class ~ ., data = xx.train,
                 method = "rf",
...
lift_results$rf <- predict(rf.fit, xx.test, type = "prob")[,"dry"]
