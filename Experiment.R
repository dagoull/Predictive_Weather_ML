...
xgbDART_grid = expand.grid(nrounds = 100, max_depth = 4, eta = 0.4, gamma = 0, subsample =1, 
                           colsample_bytree = 0.6, rate_drop = 0.6, skip_drop = 0.95, min_child_weight = 1)
...
#plot(importance, main= paste("model_xgbDART, Accuracy:", round(max(model_xgbDART$results$Accuracy),2)), ylab="ranking of predictors" )
...
models_compare <- resamples(list(lmt = LMT.fit, lda = lda.fit, rf=rf.fit, gbm = model_gbm, 
                                 glm = glm.fit, svm = svmPoly,
                                 XGBoost=model_xgbDART))

models_compare <- resamples(list(LMT = LMT.fit, RF=rf.fit, 
                                 GBM = model_gbm 
                                 ))
...
comparaciones %>% filter((modeloA == "RF") | (modeloA == "XGBDART" & modeloB == "RF"))
...
bwplot(models_compare, ylab = "Result Model Training",
       box.ratio = 1)
