...
set.seed(123)
boot.ctrl = trainControl ( method = "cv" , number = 10, sampling = "up", classProbs = TRUE )
boot.ctrl <- trainControl(method="repeatedcv", number=10, repeats=10, sampling = "up", classProbs = TRUE )
...
confusionMatrix(reference = xx.test$Class, data = predicted, mode='everything', positive='dry')
...
xgbDART_grid = expand.grid(nrounds = 300, max_depth = 5, eta = 0.3, gamma = 0, subsample =0.7, 
colsample_bytree = 0.6, rate_drop = 0.5, skip_drop = 0.05, min_child_weight = 1)
...
ggplot(rf_ranger, highlight = TRUE) + labs(title = "Evolución del accuracy del modelo Random Forest") + 
