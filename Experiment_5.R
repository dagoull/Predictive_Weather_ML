...
xx.train <-xx.train[, -c(13)]
xx.test<-  xx.test[, -c(13)]
...
fitControl <- trainControl(
  ...
  savePredictions = 'final',       # saves predictions for optimal tuning parameter
  classProbs = T,                  # should class probabilities be returned
  sampling = "smote",               # "rose", "down", "up", "smote" 
  summaryFunction=twoClassSummary  # results summary function
) 

# Step 1: Tune hyper parameters by setting tuneLength
set.seed(123)
model_mars2 = train(Class ~ ., data = xx.train, method='earth', preProc = c("corr"), tuneLength = 40,  metric='ROC', trControl = fitControl)
model_mars2
plot(model_mars2)
...
set.seed(123)
model_mars3 = train(Class ~ ., data = xx.train, method='earth', preProc = c("corr"), metric='ROC', tuneGrid = marsGrid, trControl = fitControl)
model_mars3
plot(model_mars3)
...
model_adaboost = train(Class ~ ., data = xx.train, method='adaboost', preProc = c("corr"), metric='ROC', tuneLength=4, trControl = fitControl)
model_adaboost
plot(model_adaboost)
...
model_rf = train(Class ~ ., data = xx.train, method='rf',preProc = c("corr"), metric='ROC', tuneLength=11, trControl = fitControl)
model_rf
plot(model_rf)
...
model_xgbDART = train(Class ~ ., data = xx.train, method='xgbDART', preProc = c("corr"), metric='ROC', tuneLength=5, trControl = fitControl, verbose=F)
model_xgbDART
...
# Train the model using MARS
model_svmRadial = train(Class ~ ., data = xx.train,  method='svmRadial', preProc = c("corr"), metric='ROC', tuneLength=25, trControl = fitControl)
model_svmRadial
...
## Control de la Tecnica de Remuestreo: 10-fold CV
boot.ctrl = trainControl ( method = "cv" , number = 10, classProbs = TRUE )
boot.ctrl <- trainControl(method="repeatedcv", number=10, repeats=3, classProbs = TRUE, sampling = "up" )
set.seed(123)
...
predicted2 <- predict(glm.fit, xx.test)
...
set.seed(123)
boot.ctrl = trainControl ( method = "cv" , number = 10, classProbs = TRUE, sampling = "smote" )
...
#-----------------------------------------------------------------------------------------------------
## Control de la Tecnica de Remuestreo: 10-fold CV
boot.ctrl = trainControl ( method = "cv" , number = 10, sampling = "up")
...
fitControl = trainControl ( method = "cv" , number = 5, sampling = "up")

xgbDART_grid = expand.grid(nrounds = 250, max_depth = 4, eta = 0.3, gamma = 0, subsample =0.5, 
                           colsample_bytree = 0.8, rate_drop = 0.01, skip_drop = 0.95, min_child_weight = 1)

xgbDART_grid = expand.grid(nrounds = 50, max_depth = 6, eta = 0.4, gamma = 0, subsample =1, 
                           colsample_bytree = 0.5, rate_drop = 0.01, skip_drop = 0.95, min_child_weight = 1)

#The final values used for the model were nrounds = 50, max_depth = 6, eta = 0.4, gamma = 0, subsample =
#  1, colsample_bytree = 0.8, rate_drop = 0.5, skip_drop = 0.95 and min_child_weight = 1.
...
model_xgbDART
plot(model_xgbDART, main= paste("eXtreme Gradient Boosting (xgbD
                                ART), Accuracy:", round(max(model_xgbDART$results$Accuracy),2), "Kappa: ", round(max(model_xgbDART$results$Kappa),2)))
summary(model_xgbDART)
...
fitControl = trainControl ( method = "cv" , sampling = "up", number = 10)
set.seed(123)
rf.fit = train ( Class ~ ., data = xx.train,
                 method = "rf",
